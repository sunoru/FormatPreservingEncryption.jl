...
macro _ff3_impl(ctx, input, is_encrypt)
    quote
        ctx = $(esc(ctx))
        input = $(esc(input))
        aes_key = ctx.aes_key
...
function AESNI.encrypt(ctx::FF3, plain::AbstractArray{<:Integer})
    @_ff3_impl(ctx, plain, true)
end

function AESNI.decrypt(ctx::FF3, cipher::AbstractArray{<:Integer})
    @_ff3_impl(ctx, cipher, false)
end
